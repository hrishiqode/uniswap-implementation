...
async function main() {
  ...
  const ABI_WETH = JSON.parse(
    fs.readFileSync(
      path.resolve(
          __dirname, "../artifacts/contracts/IWrapper.sol/IWrapper.json"
        ),
      "utf-8"
      )
    ).abi;
  ...
  const swapTokens = await (await ethers.getContractFactory("SwapTokens")).
    deploy(
      "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2",
      "0xdAC17F958D2ee523a2206206994597C13D831ec7",
